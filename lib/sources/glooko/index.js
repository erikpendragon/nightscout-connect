...
function glookoSource (opts, axios) {
  ...
  var impl = {
    ...
    transformData (batch) {
      ...
      // Both carry the settings snapshot's guid. Suppress anything already in
      // Nightscout: the note is dated when it is written, so without this it
      // would be a fresh timestamp every poll and would pile up forever, and
      // an unchanged profile would be rewritten on every cycle.
      ...
      // Asked for settings, got a response, and made nothing of it. Worth
      // saying out loud - the alternative is a mode that looks enabled and
      // silently does nothing.
      if (settingsResponse && !profiles.length && !settingsNotes.length) {
