...
// The single timezone implementation, shared with convert.js. Intl-based,
// so it has no dependency that can vanish and silently downgrade this to
// a fixed offset.
var tzlib = require('./timezone');
var Defaults = {
...
// The consumer twin of the partner gateway's /api/v2/external/pumps/settings.
// The /external/ path needs a partner JWT and API key and answers 401 to an
// email-and-password session; this one answers like every other v2 route.
// Therapy settings are read from DeviceSettings below; this is kept for the
// documented v2 shape (see the Direct API docs, Insulin Pump Data > Pump Settings).
, PumpSettings: '/api/v2/pumps/settings'
, PumpEvents: '/api/v2/pumps/events'
, PumpAlarms: '/api/v2/pumps/alarms'
// Basal suspensions. On the account this was built against every record is
// type "manual" - the Omnipod 5 algorithm's own suspensions are not in this
// collection, only in the v3 graph's basalBarAutomatedSuspend series.
, PumpSuspendBasals: '/api/v2/pumps/suspend_basals'
// Manual-mode collections. Empty over 400 days on the Omnipod 5 account this
// was built against, so their record shapes are taken from the official Direct
// API dictionaries and the sibling collections, not from a live capture.
, PumpTempBasals: '/api/v2/pumps/temporary_basals'
, PumpExtendedBoluses: '/api/v2/pumps/extended_boluses'
// Glucose readings stored on the pump (manual entries, or a linked meter), and
// insulin/carb events logged in the CGM app. All three were empty over 90 days
// on the account this was built against; shapes inferred from the official
// dictionaries, with snake_case fallbacks because cgm/readings is snake_case.
, PumpReadings: '/api/v2/pumps/readings'
, CgmInsulinEvents: '/api/v2/cgm/insulin_events'
, CgmCarbsEvents: '/api/v2/cgm/carbs_events'
// Meter (BGM) readings and blood pressures. Both empty on the account this was
// built against; blood_pressures answers under the key bloodPressures.
, MeterReadings: '/api/v2/readings'
, BloodPressures: '/api/v2/blood_pressures'
, DeviceSettings: '/api/v3/devices_and_settings'
// Daily activity (steps from a phone or watch via Validic) exists only here, as
// history entries of type validic_routines, one per day. The same endpoint also
// carries copies of collections already fetched over v2; those are ignored.
// v3 takes patient/startDate/endDate and answers 422 to the v2 cursor params.
, V3Histories: '/api/v3/users/summary/histories'
...
// What to do with the pump's own therapy settings.
//   off      - do not fetch them at all. The default: no extra request per poll.
//   propose  - fetch, and post a Note saying what the pump holds. Writes nothing.
//   override - fetch, and write them into the Nightscout profile.
// Anything unrecognised is treated as off, because the failure mode of guessing
// wrong here is writing therapy values nobody asked for.
function profileSyncFor (value) {
  var mode = String(value == null ? '' : value).trim().toLowerCase();
  return [ 'off', 'propose', 'override' ].includes(mode) ? mode : 'off';
}
...
function readingToEntry (timestampDelta, timezone, reading) {
  ...
  var date = tzlib.timestampWithOffset(timestamp, timestampDelta, timezone);
  return {
    type: 'sgv',
...
function v3PointToEntry (timestampDelta, meterUnits, point) {
  ...
  var date = tzlib.timestampWithOffset(timestamp, timestampDelta);
  return {
    type: 'sgv',
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      */      
      ...
      // The pump's own therapy settings. A clinician changes these a handful
      // of times a year, so they are only requested when the feature is on -
      // otherwise every poll would carry a request that answers the same thing
      // it did five minutes ago.
      function settingsFetcher ( ) {
        // Opt in explicitly. An unset or unrecognised mode does nothing, so a
        // caller that never heard of this feature cannot end up fetching
        // therapy settings by accident.
        if (opts.glookoProfileSync !== 'propose' && opts.glookoProfileSync !== 'override') {
          return Promise.resolve(null);
        }
        return fetcher(Defaults.DeviceSettings, { }).catch(function (e) {
          console.log('GLOOKO device settings fetch failed',
                      e && e.response ? e.response.status : e && e.message);
          return null;
        });
      }

      // Daily step records live only in the v3 histories. Same 14-day window as
      // the other sparse collections; without dates the endpoint returns the
      // account's entire history, thousands of entries every poll.
      function historiesFetcher ( ) {
        var pc = getGlookoCode(session);
        if (!pc) { return Promise.resolve({ histories: [ ] }); }
        var wideStart = new Date(new Date( ).getTime( ) - 14 * 24 * 60 * 60 * 1000);
        return fetcher(Defaults.V3Histories, {
          patient: pc,
          startDate: wideStart.toISOString( ),
          endDate: new Date( ).toISOString( )
        }).catch(function (e) {
          console.log('GLOOKO histories fetch failed',
                      e && e.response ? e.response.status : e && e.message);
          return { histories: [ ] };
        });
      }
      ...
      const v2Fetches = pumpBasalsUrl && pumpBolusUrl && cgmReadingsUrl
        ? [
            ...
            settingsFetcher(),
            eventsFetcher(Defaults.PumpSuspendBasals),
            eventsFetcher(Defaults.PumpTempBasals),
            eventsFetcher(Defaults.PumpExtendedBoluses),
            eventsFetcher(Defaults.PumpReadings),
            eventsFetcher(Defaults.CgmInsulinEvents),
            eventsFetcher(Defaults.CgmCarbsEvents),
            eventsFetcher(Defaults.MeterReadings),
            eventsFetcher(Defaults.BloodPressures),
            historiesFetcher(),
        ...
        : [
            ...
            Promise.resolve({ suspendBasals: [] }),
            Promise.resolve({ temporaryBasals: [] }),
            Promise.resolve({ extendedBoluses: [] }),
            Promise.resolve({ readings: [] }),
            Promise.resolve({ insulinEvents: [] }),
            Promise.resolve({ carbsEvents: [] }),
            Promise.resolve({ readings: [] }),
            Promise.resolve({ bloodPressures: [] }),
      ...
      return Promise.all(v2Fetches).then(function (results) {
         ...
         var some = {
            ...
            deviceSettings: results[8] || null,
            suspendBasals: ((results[9] && results[9].suspendBasals) || [ ]).filter(function (s) {
              if (!s || s.softDeleted || !s.pumpTimestamp || !s.guid) { return false; }
              return !seen_guids.has(s.guid);
            }),
            temporaryBasals: ((results[10] && results[10].temporaryBasals) || [ ]).filter(function (t) {
              if (!t || t.softDeleted || !(t.pumpTimestamp || t.timestamp) || !t.guid) { return false; }
              return !seen_guids.has(t.guid);
            }),
            extendedBoluses: ((results[11] && results[11].extendedBoluses) || [ ]).filter(function (b) {
              if (!b || b.softDeleted || !(b.pumpTimestamp || b.timestamp) || !b.guid) { return false; }
              return !seen_guids.has(b.guid);
            }),
            pumpReadings: ((results[12] && results[12].readings) || [ ]).filter(function (r) {
              if (!r || r.softDeleted || r.soft_deleted || !(r.pumpTimestamp || r.timestamp) || !r.guid) { return false; }
              return !seen_guids.has(r.guid);
            }),
            cgmInsulinEvents: ((results[13] && results[13].insulinEvents) || [ ]).filter(function (e) {
              if (!e || e.softDeleted || e.soft_deleted || !e.guid) { return false; }
              if (!(e.systemTime || e.system_time || e.eventTime || e.event_time || e.displayTime || e.display_time || e.timestamp)) { return false; }
              return !seen_guids.has(e.guid);
            }),
            cgmCarbsEvents: ((results[14] && results[14].carbsEvents) || [ ]).filter(function (e) {
              if (!e || e.softDeleted || e.soft_deleted || !e.guid) { return false; }
              if (!(e.systemTime || e.system_time || e.eventTime || e.event_time || e.displayTime || e.display_time || e.timestamp)) { return false; }
              return !seen_guids.has(e.guid);
            }),
            meterReadings: ((results[15] && (results[15].readings || results[15].meterReadings)) || [ ]).filter(function (r) {
              if (!r || r.softDeleted || r.soft_deleted || !r.timestamp || !r.guid) { return false; }
              return !seen_guids.has(r.guid);
            }),
            bloodPressures: ((results[16] && (results[16].bloodPressures || results[16].blood_pressures)) || [ ]).filter(function (b) {
              if (!b || b.softDeleted || b.soft_deleted || !b.timestamp || !b.guid) { return false; }
              return !seen_guids.has(b.guid);
            }),
            // one record per day; the entry's guid is the stable identity
            activityDays: ((results[17] && results[17].histories) || [ ]).filter(function (h) {
              if (!h || h.type !== 'validic_routines' || h.softDeleted) { return false; }
              var item = h.item;
              var guid = h.guid || (item && item.guid);
              if (!item || item.softDeleted || !item.timestamp || !guid) { return false; }
              return !seen_guids.has(guid);
            }).map(function (h) { return { ...h.item, guid: h.guid || h.item.guid }; }),
            // the guids already in Nightscout, so the transform can suppress a
            // settings note for a snapshot that has been reported before
         ...
         if (some.suspendBasals && some.suspendBasals.length) {
           console.log('GLOOKO new suspends:',
                       JSON.stringify(some.suspendBasals.map(function (s) { return s.type + ' ' + s.duration + 's'; })));
         }
         if (some.temporaryBasals && some.temporaryBasals.length) {
           console.log('GLOOKO new temp basals:', some.temporaryBasals.length,
                       'first record keys:', JSON.stringify(Object.keys(some.temporaryBasals[0])));
         }
         if (some.extendedBoluses && some.extendedBoluses.length) {
           console.log('GLOOKO new extended boluses:', some.extendedBoluses.length,
                       'first record keys:', JSON.stringify(Object.keys(some.extendedBoluses[0])));
         }
         [ 'pumpReadings', 'cgmInsulinEvents', 'cgmCarbsEvents', 'meterReadings', 'bloodPressures', 'activityDays' ].forEach(function (k) {
           if (some[k] && some[k].length) {
             console.log('GLOOKO new ' + k + ':', some[k].length,
                         'first record keys:', JSON.stringify(Object.keys(some[k][0])));
           }
         });
    ...
    transformData (batch) {
      ...
      // The pump's own therapy settings, when this instance asked for them.
      // In override mode they become a Nightscout profile; in propose mode a
      // note describing them, and nothing is written. Values are left in the
      // Glooko account's units - see generate_nightscout_profile.
      var settingsResponse = batch && batch.deviceSettings;
      var settingsOpts = {
        units: opts.glookoUnits
      , timezone: opts.glookoTimezone
      , activePumpGuid: settingsResponse && settingsResponse.activePumpGuid
      };
      var settingsTree = settingsResponse && settingsResponse.deviceSettings;
      var profiles = opts.glookoProfileSync === 'override'
        ? helper.generate_nightscout_profile(settingsTree, settingsOpts) : [ ];
      var settingsNotes = opts.glookoProfileSync === 'propose'
        ? helper.generate_settings_note(settingsTree, settingsOpts) : [ ];
      // Settings arrived and nothing in them parsed. Glooko carries many pumps
      // and this driver has been proven against few, so this is an expected
      // outcome rather than a bug - but it has to be visible. Silently doing
      // nothing would leave the wearer believing the profile was being kept in
      // step when it is not. The note says the profile is theirs to maintain;
      // the log line gives whoever debugs it the shape that was not understood.
      var wantsSettings = opts.glookoProfileSync === 'propose'
                       || opts.glookoProfileSync === 'override';
      if (wantsSettings && settingsResponse && !profiles.length && !settingsNotes.length) {
        settingsNotes = helper.generate_unsupported_note(settingsResponse, settingsOpts);
        console.log('GLOOKO device settings produced no profile; response keys:',
                    JSON.stringify(Object.keys(settingsResponse)),
                    'settings keys:',
                    JSON.stringify(settingsTree ? Object.keys(settingsTree) : null),
                    'pump guids:',
                    JSON.stringify(settingsTree && settingsTree.pumps ? Object.keys(settingsTree.pumps).length : null));
      }

      // Everything above carries the guid of whatever it describes - the
      // settings snapshot, or the pump that could not be read. Suppress
      // anything Nightscout already has: the notes are dated when they are
      // written, so without this each would be a fresh timestamp every poll
      // and would pile up forever, and an unchanged profile would be
      // rewritten on every cycle.
      var alreadySeen = batch && batch.seenGuids;
      if (alreadySeen && alreadySeen.has) {
        profiles = profiles.filter(function (p) { return !alreadySeen.has(p.glookoGuid); });
        settingsNotes = settingsNotes.filter(function (n) { return !alreadySeen.has(n.glookoGuid); });
      }
      if (settingsNotes.length) { treatments = treatments.concat(settingsNotes); }
      ...
      if (opts.glookoSkipEntries) {
        var only = { entries: [ ], treatments: treatments };
        ...
        if (profiles.length) only.profiles = profiles;
        return only;
      }
...
glookoSource.validate = function validate_inputs (input) {
  ...
  var config = {
    ...
    glookoProfileSync: profileSyncFor(input.glookoProfileSync),
    glookoUnits: input.glookoUnits || '',
    baseURL
  };
  var errors = [ ];
