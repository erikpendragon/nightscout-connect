...
function generate_v3_nightscout_entries (graph, timestampDelta) {
  ...
  return V3_CGM_SERIES
    ...
    .map(v3PointToEntry.bind(null, timestampDelta))
    .filter(Boolean);
}
  ...
  var impl = {
    ...
    transformData (batch) {
      ...
      if (!entries.length && opts.glookoUseV3Graph) {
        entries = generate_v3_nightscout_entries(batch && batch.v3Graph, opts.glookoTimezoneOffset);
      }
