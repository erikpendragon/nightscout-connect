...
function generate_nightscout_treatments(batch, timestampDelta, timezone) {
  ...
  */
  ...
  // Basal suspensions from the pump's own record: pod changes and deliberate
  // suspends. A Temp Basal at 0 U/h for the suspension's duration is what the
  // basal plugin understands. Note this is not loop behaviour - on an Omnipod 5
  // the algorithm's suspensions never appear in this collection.
  const suspendBasals = batch.suspendBasals;
  if (suspendBasals) {
    suspendBasals.forEach(function (s) {
      if (!s || !s.pumpTimestamp) { return; }
      var when = tz.timestampWithOffset(s.pumpTimestamp, timestampDelta, timezone).toISOString( );
      var treatment = {};
      treatment.eventType = 'Temp Basal';
      // Both: created_at is what the basal renderer keys on, eventTime is
      // what Nightscout derives mills from.
      treatment.eventTime = when;
      treatment.created_at = when;
      treatment.rate = 0;
      treatment.absolute = 0;
      treatment.duration = Math.round((Number(s.duration) || 0) / 6) / 10;  // seconds -> minutes
      treatment.reason = 'Pump suspended' + (s.type ? ' (' + s.type + ')' : '');
      treatment.enteredBy = 'glooko-suspend';
      treatment.glookoGuid = s.guid;
      if (s.rateAtStart != null) { treatment.glookoRateAtStart = s.rateAtStart; }
      if (s.rateAtResume != null) { treatment.glookoRateAtResume = s.rateAtResume; }
      treatments.push(treatment);
    });
  }

  // Temporary basals set by hand on the pump (manual mode only on an Omnipod 5).
  // Shape inferred from the official dictionary and the suspend record: a
  // pumpTimestamp, a duration in seconds, and either an absolute rate or a
  // percentage of the scheduled rate. An absolute rate is preferred because
  // Nightscout can render it without knowing the schedule. Until a live record
  // fixes the percentage convention, a value of 2 or less is read as a fraction
  // (0.25 = a quarter of scheduled) and anything larger as a plain percentage
  // (25 = a quarter of scheduled); both become Nightscout's relative percent.
  const temporaryBasals = batch.temporaryBasals;
  if (temporaryBasals) {
    temporaryBasals.forEach(function (t) {
      var ts = t && (t.pumpTimestamp || t.timestamp);
      if (!ts) { return; }
      var when = tz.timestampWithOffset(ts, timestampDelta, timezone).toISOString( );
      var seconds = Number(t.duration);
      if (!isFinite(seconds) && t.ended) { seconds = (new Date(t.ended).getTime( ) - new Date(ts).getTime( )) / 1000; }
      var treatment = {};
      treatment.eventType = 'Temp Basal';
      treatment.eventTime = when;
      treatment.created_at = when;
      treatment.duration = Math.round((isFinite(seconds) ? seconds : 0) / 6) / 10;  // seconds -> minutes
      var rate = Number(t.rate);
      var pct = t.percentage != null ? Number(t.percentage) : (t.percent != null ? Number(t.percent) : NaN);
      if (isFinite(rate) && t.rate !== null && t.rate !== undefined) {
        treatment.rate = rate;
        treatment.absolute = rate;
      } else if (isFinite(pct)) {
        treatment.percent = Math.round((pct <= 2 ? pct * 100 : pct) - 100);
      } else {
        return;  // neither a rate nor a percentage: nothing Nightscout can draw
      }
      treatment.reason = 'Temp basal' + (t.type || t.tempBasalType ? ' (' + (t.type || t.tempBasalType) + ')' : '');
      treatment.enteredBy = 'glooko-temp-basal';
      treatment.glookoGuid = t.guid;
      treatments.push(treatment);
    });
  }

  // Extended (dual-wave) boluses, manual mode only on an Omnipod 5. Shape
  // inferred from the official dictionary plus the live normal-bolus record:
  // the normal-bolus fields and initialDelivery, extendedDelivery,
  // extendedBolusDuration (seconds). Nightscout's Combo Bolus wants the total
  // as enteredinsulin, the now/extended split as percentages, the immediate
  // part as insulin, and the extended part as a U/h rate in relative.
  const extendedBoluses = batch.extendedBoluses;
  if (extendedBoluses) {
    extendedBoluses.forEach(function (b) {
      var ts = b && (b.pumpTimestamp || b.timestamp);
      if (!ts) { return; }
      var now = Number(b.initialDelivery);
      var ext = Number(b.extendedDelivery);
      var total = Number(b.insulinDelivered);
      if (!isFinite(total) || total <= 0) { total = (isFinite(now) ? now : 0) + (isFinite(ext) ? ext : 0); }
      if (!(total > 0)) { return; }
      if (!isFinite(now)) { now = isFinite(ext) ? Math.max(total - ext, 0) : total; }
      if (!isFinite(ext)) { ext = Math.max(total - now, 0); }
      var seconds = Number(b.extendedBolusDuration != null ? b.extendedBolusDuration : b.duration);
      var minutes = Math.round((isFinite(seconds) ? seconds : 0) / 6) / 10;
      var when = tz.timestampWithOffset(ts, timestampDelta, timezone).toISOString( );
      var treatment = {};
      treatment.eventType = 'Combo Bolus';
      treatment.eventTime = when;
      treatment.created_at = when;
      treatment.enteredinsulin = total;
      treatment.insulin = now;
      treatment.splitNow = Math.round(now / total * 100);
      treatment.splitExt = 100 - treatment.splitNow;
      treatment.duration = minutes;
      if (minutes > 0) { treatment.relative = Math.round(ext / (minutes / 60) * 100) / 100; }  // U/h over the extended part
      var carbs = Number(b.carbsInput) || 0;
      if (carbs > 0) { treatment.carbs = carbs; }
      treatment.enteredBy = 'glooko-extended-bolus';
      treatment.glookoGuid = b.guid;
      treatments.push(treatment);
    });
  }

  // Glucose readings stored on the pump - manual entries into the controller,
  // or a linked meter. A BG Check treatment is what Nightscout has for a
  // finger-stick value, and treatments flow even where entries are skipped
  // because CGM arrives from another source. The pump stores glucose as
  // mg/dL x 100 (bloodGlucoseInput on boluses is the same), so anything at or
  // above 1000 is scaled down; a small value with mmol units is converted.
  function glucoseMgdl (r) {
    var v = Number(r.value != null ? r.value : (r.glucoseValue != null ? r.glucoseValue : r.bg_value));
    if (!isFinite(v) || v <= 0) { return null; }
    if (v >= 1000) { return Math.round(v / 100); }
    var units = String(r.units || r.meterUnits || r.meter_units || '').toLowerCase();
    if (units.indexOf('mmol') === 0 || (!units && v < 40)) { return Math.round(v * 18.0182); }
    return Math.round(v);
  }
  const pumpReadings = batch.pumpReadings;
  if (pumpReadings) {
    pumpReadings.forEach(function (r) {
      var ts = r && (r.pumpTimestamp || r.timestamp);
      if (!ts) { return; }
      var mgdl = glucoseMgdl(r);
      if (mgdl == null) { return; }
      var treatment = {};
      treatment.eventType = 'BG Check';
      treatment.eventTime = tz.timestampWithOffset(ts, timestampDelta, timezone).toISOString( );
      treatment.glucose = mgdl;
      treatment.units = 'mg/dl';
      treatment.glucoseType = 'Finger';
      if (r.type) { treatment.notes = 'Pump reading (' + r.type + ')'; }
      treatment.enteredBy = 'glooko-pump-reading';
      treatment.glookoGuid = r.guid;
      treatments.push(treatment);
    });
  }

  // Meter (BGM) readings: the same BG Check shape as pump readings, the same
  // glucose scaling, plus the meal tag when the meter or the user supplied one.
  const meterReadings = batch.meterReadings;
  if (meterReadings) {
    meterReadings.forEach(function (r) {
      if (!r || !r.timestamp) { return; }
      var mgdl = glucoseMgdl(r);
      if (mgdl == null) { return; }
      var tag = r.mealTag || r.meal_tag || r.meterMealTag;
      var treatment = {};
      treatment.eventType = 'BG Check';
      treatment.eventTime = tz.timestampWithOffset(r.timestamp, timestampDelta, timezone).toISOString( );
      treatment.glucose = mgdl;
      treatment.units = 'mg/dl';
      treatment.glucoseType = 'Finger';
      treatment.notes = 'Meter reading' + (tag && tag !== 'none' && typeof tag === 'string' ? ' (' + tag + ' meal)' : '');
      treatment.enteredBy = 'glooko-meter-reading';
      treatment.glookoGuid = r.guid;
      treatments.push(treatment);
    });
  }

  // Blood pressures. Nightscout has no record type for these, so they are Notes
  // with the numbers in the text and in glookoBloodPressure for readers.
  const bloodPressures = batch.bloodPressures;
  if (bloodPressures) {
    bloodPressures.forEach(function (b) {
      if (!b || !b.timestamp) { return; }
      var sys = Number(b.systolic), dia = Number(b.diastolic);
      if (!(sys > 0) || !(dia > 0)) { return; }
      var pulse = Number(b.pulse != null ? b.pulse : b.heartRate);
      var units = b.units || 'mmHg';
      var treatment = {};
      treatment.eventType = 'Note';
      treatment.eventTime = tz.timestampWithOffset(b.timestamp, timestampDelta, timezone).toISOString( );
      treatment.notes = 'Blood pressure ' + sys + '/' + dia + ' ' + units + (pulse > 0 ? ', pulse ' + pulse : '');
      treatment.enteredBy = 'glooko-blood-pressure';
      treatment.glookoGuid = b.guid;
      treatment.glookoBloodPressure = { systolic: sys, diastolic: dia, units: units };
      if (pulse > 0) { treatment.glookoBloodPressure.pulse = pulse; }
      treatments.push(treatment);
    });
  }

  // Daily activity from the v3 histories: one validic_routines record per day,
  // stamped at the local end of day, carrying the day's steps from a phone or
  // watch. This is a daily total, not an exercise session, so it becomes an
  // Exercise treatment at that end-of-day moment with the numbers in the note
  // and in glookoActivity. Fields other than steps (distance, calories, floors,
  // elevation, duration) are carried when the source supplies them.
  const activityDays = batch.activityDays;
  if (activityDays) {
    activityDays.forEach(function (a) {
      if (!a || !a.timestamp) { return; }
      var steps = Math.round(Number(a.steps));
      if (!(steps > 0)) { return; }
      // The record is stamped at the day's end, so a stamp still in the future
      // is today's running total. Its guid never changes, so importing it now
      // would freeze a partial count in Nightscout for good. Wait for midnight.
      var endOfDay = tz.timestampWithOffset(a.timestamp, timestampDelta, timezone);
      if (endOfDay.getTime( ) > Date.now( )) { return; }
      var treatment = {};
      treatment.eventType = 'Exercise';
      treatment.eventTime = endOfDay.toISOString( );
      var seconds = Number(a.duration);
      if (seconds > 0) { treatment.duration = Math.round(seconds / 6) / 10; }
      treatment.notes = 'Daily steps: ' + steps + (a.source ? ' (' + a.source + ')' : '');
      treatment.enteredBy = 'glooko-activity';
      treatment.glookoGuid = a.guid;
      var activity = { steps: steps, day: String(a.timestamp).slice(0, 10) };
      [ 'distance', 'calories', 'floors', 'elevation', 'source' ].forEach(function (k) {
        if (a[k] != null && a[k] !== '') { activity[k] = a[k]; }
      });
      treatment.glookoActivity = activity;
      treatments.push(treatment);
    });
  }

  // Insulin and carb events logged in the CGM app. Imported as Note, not as
  // bolus or carb treatments: on a pump account the pump already records the
  // bolus and its carbsInput, and a second entry would count twice into IOB and
  // COB - the same reason food is imported as Note. systemTime is real UTC and
  // is used as-is when present; displayTime is local-stamped-as-UTC and goes
  // through the timezone conversion like everything else.
  function cgmEventTime (e) {
    var sys = e.systemTime || e.system_time;
    if (sys) { return new Date(sys).toISOString( ); }
    var local = e.eventTime || e.event_time || e.displayTime || e.display_time || e.timestamp;
    return tz.timestampWithOffset(local, timestampDelta, timezone).toISOString( );
  }
  const cgmInsulinEvents = batch.cgmInsulinEvents;
  if (cgmInsulinEvents) {
    cgmInsulinEvents.forEach(function (e) {
      var units = Number(e.insulin);
      if (!isFinite(units) || units <= 0) { return; }
      var treatment = {};
      treatment.eventType = 'Note';
      treatment.eventTime = cgmEventTime(e);
      treatment.notes = 'Insulin logged in CGM app: ' + units + ' U';
      treatment.enteredBy = 'glooko-cgm-insulin';
      treatment.glookoGuid = e.guid;
      treatment.glookoCgmInsulin = units;
      treatments.push(treatment);
    });
  }
  const cgmCarbsEvents = batch.cgmCarbsEvents;
  if (cgmCarbsEvents) {
    cgmCarbsEvents.forEach(function (e) {
      var grams = Number(e.carbs);
      if (!isFinite(grams) || grams <= 0) { return; }
      var treatment = {};
      treatment.eventType = 'Note';
      treatment.eventTime = cgmEventTime(e);
      treatment.notes = 'Carbs logged in CGM app: ' + grams + ' g';
      treatment.enteredBy = 'glooko-cgm-carbs';
      treatment.glookoGuid = e.guid;
      treatment.glookoCgmCarbs = grams;
      treatments.push(treatment);
    });
  }
...
// Glooko carries many pumps and this driver has been proven against few. When
// settings arrive but nothing in them parses, say so where the user will see
// it rather than in a container log. A driver that quietly does nothing is
// worse than one that says it cannot help: the profile then stays
// hand-maintained on purpose rather than by accident.
// Field names only, never values. An unreadable pump is the one case where the
// shape itself is the useful thing to report: whoever owns that pump can paste
// the note straight into an issue, and it is exactly the fixture needed to add
// support. Nothing identifying can ride along - the levels keyed by device guid
// and by snapshot time are skipped by reading only the newest snapshot's
// contents, and every name is filtered below.
function safe_field_name (name) {
  if (typeof name !== 'string') return false;
  // A field name is a plain identifier. A guid or an ISO timestamp carries
  // hyphens and colons, a serial carries a run of digits, and an id is not a
  // word - so none of them survive this.
  if (!/^[A-Za-z][A-Za-z0-9_]{1,39}$/.test(name)) return false;
  if (/[0-9]{3}/.test(name)) return false;
  return (name.match(/[A-Za-z]/g) || [ ]).length >= 3;
}

function collect_field_names (node, depth, into) {
  if (!node || typeof node !== 'object' || depth > 6 || into.length >= 200) return;
  if (Array.isArray(node)) {
    // One element is enough - every entry in these arrays shares a shape.
    if (node.length) collect_field_names(node[0], depth, into);
    return;
  }
  Object.keys(node).forEach(function (name) {
    // A rejected key is still walked through, never reported. A map keyed by
    // guid or by timestamp is exactly where the interesting shape hides, so
    // dropping the branch would throw away the part worth having.
    if (safe_field_name(name) && into.indexOf(name) < 0) into.push(name);
    collect_field_names(node[name], depth + 1, into);
  });
}

function settings_field_names (response) {
  var found = newest_snapshot(response && response.deviceSettings,
                              response && response.activePumpGuid);
  var root = (found && found.settings) || response;
  var names = [ ];
  collect_field_names(root, 0, names);
  return names.sort( );
}

function generate_unsupported_note (response, opts) {
  opts = opts || { };
  if (!response || typeof response !== 'object') return [ ];

  var pumps = (response.devices || [ ]).filter(function (d) {
    return d && d.type === 'pump';
  });
  var named = pumps.map(function (d) {
    return [ d.brand, d.model ].filter(Boolean).join(' ').trim();
  }).filter(Boolean);
  var label = named.length ? named.join(', ') : 'the pump on this account';

  // Stable per pump, so this is said once rather than every poll, and said
  // again if the wearer changes to a different pump.
  var key = response.activePumpGuid
    || pumps.map(function (d) { return d.guid || d.id; }).filter(Boolean).join('+')
    || 'unknown';

  var treatment = { };
  treatment.eventType = 'Note';
  treatment.eventTime = new Date( ).toISOString( );
  treatment.notes = 'Could not read therapy settings for ' + label + ' from Glooko. '
    + 'The Nightscout profile has to be maintained by hand - insulin duration, '
    + 'carb ratio, correction factor, targets and basal schedule.';

  var fields = settings_field_names(response);
  var shown = fields.slice(0, 60);
  if (shown.length) {
    treatment.notes += ' Adding support for this pump means teaching the'
      + ' converter the shape below; field names only, no values: '
      + shown.join(', ')
      + (fields.length > shown.length
         ? ', and ' + (fields.length - shown.length) + ' more' : '')
      + '.';
  }

  treatment.enteredBy = 'glooko-settings';
  treatment.glookoGuid = 'devicesettings-unreadable-' + key;
  return [ treatment ];
}
...
module.exports.generate_unsupported_note = generate_unsupported_note;
module.exports.settings_field_names = settings_field_names;
