...
function nightscoutRestAPI (config, axios) {
  ...
  function record_devicestatus (data) {
    ...
    if (bookmark.lastDeviceStatus) {
      data = data.filter(function (d) {
        return d && d.created_at && d.created_at > bookmark.lastDeviceStatus;
      });
      if (!data.length) {
        return Promise.resolve( );
      }
    }
    var newest = data.reduce(function (acc, d) {
      return (d && d.created_at && d.created_at > acc) ? d.created_at : acc;
    }, bookmark.lastDeviceStatus || '');
    var headers = { 'API-SECRET': apiHash };
    return http.post('/api/v1/devicestatus.json', data, { headers }).then((resp) => {
      bookmark.lastDeviceStatus = newest;
  ...
  record_batch.gap_for = function ( ) {
    ...
    return Promise.all([
      ...
      // Same reasoning for devicestatus: without seeding, every restart posts
      // one more copy of the IOB record it already sent.
      http.get('/api/v1/devicestatus.json', { params: { count: 1 }, headers })
        .then((resp) => {
          var newest = (resp.data || [])[0];
          if (newest && newest.created_at) {
            bookmark.lastDeviceStatus = newest.created_at;
            console.log("SEEDED devicestatus watermark", bookmark.lastDeviceStatus);
          }
        }).catch(function ( ) { }),
    ]).then(( ) => {
